...
<%-- <script src="<%=basePath%>static/js/IM-index.js"></script> --%>
<link rel="stylesheet" type="text/css" href="<%=basePath%>static/css/IM-index.css" />
<script src="<%=basePath%>static/js/super.js"></script>
<style>
html, body {
	height: 100%;
}
...
$(function(){
	$.ajax({
		...
		var v = "<option value=''>请选择</option>"
			for(var i = 0; i <result.length; i ++){
			 v += "<option value="+result[i].code + ">" + result[i].pname + "</option>";
	...
	$("#shen").change(function(){
		...
		$.get("<%=basePath%>QueryCities",{"code":$("#shen").val()},function(result){
			$("#shi").empty();
			var v = "<option value=''>请选择</option>"
			for(var i = 0; i <result.length; i ++){
			 v += "<option value="+result[i].code + ">" + result[i].cname + "</option>";
	...
	$("#shi").change(function(){
		$.getJSON("<%=basePath%>QueryTown",{"code":$("#shi").val()},function(data){
		$("#xian").empty();
			var v = "<option value=''>请选择</option>"
			for(var i = 0; i <data.length; i ++){
			 v += "<option value="+data[i].code + ">" + data[i].tname + "</option>";
...
<script type="text/javascript">
	$(function(){
		$.ajax({
			url:"<%=basePath%>QueryAllCropSpecies",
			type:"post",
			data:{
...
$(function(){
	$.ajax({
		...
		success:function(data){
			...
			var v = '<option value="">请选择</option>'
			for(var i = 0; i<data.result.length; i++){
				v += '<option value='+data.result[i].tid+'>'+data.result[i].toxinType+'</option>'
			}
			$("#toxin_type").append(v)
		}
	});
...
<script type="text/javascript">
	$(function(){
		findallsampleInfo();
		function findallsampleInfo(){
			
			$.ajax({
				url:"<%=basePath%>queryallinfo",
				type:"post",
				data:{
					...
					if(data.code == 200){
						var v = '<table><thead> <th class="w1"><input type="checkbox" id="checkbox1" value="">全选</th>';
			 			...
			 			for(var i = 0; i <data.result.list.length; i ++){
							 v+= '<tr>';
							 v+='<td><input class="testyangpin" value="'+data.result.list[i].id+'" type="checkbox"></td>';
							 v+=' <td class="ybbh">'+data.result.list[i].sampleId+'</td>';
							 v+='<td>'+data.result.list[i].addressProvince.pname + data.result.list[i].addressCity.cname + data.result.list[i].addressTown.tname+'</td>';
							 v+='<td>'+data.result.list[i].cropSpecies.cropSpecies+'</td>';
							 v+=' <td>'+data.result.list[i].samplingTime+'</td>';
							 v+='<td>'+data.result.list[i].inputTime+'</td>';
							 v+=' <td>'+data.result.list[i].pollutionRate+'</td>';
							 var toxin = data.result.list[i].infos;
							 v+='<td>';
							 for(var j = 0; j <toxin.length; j ++){
							 	v+=toxin[j].info.toxinType;
							 }
							 v+='</td>';
							 v+='<td><a href="editIM.html">编辑</a> |<a >删除</a></td> </tr></stbody>';
						}
						v+=' </table>';
						$("#tablediv").append(v);
...
<body onload="load()">
		...
		<div class="im-content">
			<div class="im-heade">
				...
				<div class="imhead-select">
					<ul>
						<li class="bh"><label>样品编号</label> <input maxlength="15"
							onkeyup="this.value=this.value.replace(/\s+/g,'')" id="sample_id"
							value="" type="text"></li>
						<li class="wrl mright"><label>污染率</label> <select
							id="wuranlv">
						...
						<li class="time"><label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;时间</label>
							<input id="qaz" value="" type="hidden"> <select
						...
						</select> <span> &nbsp;月 </span> <select name="day" id="day" class="day">
								<option value="0">请选择</option>
						</select> <span>&nbsp; 日</span></li>
						<li class="w mright yu"><label>主要污染菌种类</label> <select
							name="toxin_type" class="mainSpecies" id="toxin_type">
								<option value="">请选择</option>
						</select></li>
						<li class="w mright yu"><lable>农作物种类</lable> <select
							name="crop_species" class="nzw_spacies" id="crop_species">
								<option value="">请选择</option>
						</select></li>
						<li class="place w"><lable>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;地点</lable>
							...
							<select name="city" class="city" id="shi" onchange="findtown(this.value)">
								<option value="">请选择</option>
							</select> <span> &nbsp;市</span>
							<select name="town" class="county" id="xian">
								<option value="">请选择</option>
							</select> <span> &nbsp;县</span></li>
					</ul>
					<div class="soso">
						<a	id="sousuo"  onclick="sousuo">搜索</a>
					</div>
				</div>
				<div class="oper">
					<ul>
						<li class="btn-top">
							<a href="editIM-add.html" class="dt">新增单条</a>
							...
							<a href="javascript:void(0)" class="dc">信息导出</a>
						</li>
					</ul>
				</div>
			</div>
	...
	<link type="text/css" rel="stylesheet" href="<%=basePath %>static/css/paging.css">
	<style type="text/css">
.color a {
	padding: 5px;
	color: #4c4743;
...
/* #h1 {
	...
	<script type="text/javascript"
		src="<%=basePath %>/static/js/jquery1.11.3-jquery.min.js"></script>
